...
from servidor.sistema.negocio.ingreso.model import Ingreso
...
class IngresoManager(SuperManager):

    def __init__(self, db):
        super().__init__(Ingreso, db)
    ...
    def insert(self, objeto):
        ...
        b = Bitacora(fkusuario=objeto.user, ip=objeto.ip, accion="Registró ingreso.", fecha=fecha, tabla="negocio_ingreso", identificador=a.id)
        super().insert(b)
        return a
    ...
    def update(self, objeto):
        ...
        b = Bitacora(fkusuario=objeto.user, ip=objeto.ip, accion="Modificó ingreso.", fecha=fecha, tabla="negocio_ingreso", identificador=a.id)
        super().insert(b)
        return a

    def state(self, id, estado, user, ip):
        x = self.db.query(self.entity).filter(self.entity.id == id).one()
        mensaje = "Habilitó ingreso" if estado else "Deshabilitó ingreso"
        x.estado = estado

        fecha = BitacoraManager(self.db).fecha_actual()
        b = Bitacora(fkusuario=user, ip=ip, accion=mensaje, fecha=fecha, tabla="negocio_ingreso", identificador=id)
        super().insert(b)
        self.db.merge(x)
        self.db.commit()
        return x
    ...
    def delete(self, id, user, ip):
        ...
        fecha = BitacoraManager(self.db).fecha_actual()
        b = Bitacora(fkusuario=user, ip=ip, accion="Eliminó ingreso", fecha=fecha, tabla="negocio_ingreso", identificador=id)
        super().insert(b)
        self.db.merge(x)
        self.db.commit()
