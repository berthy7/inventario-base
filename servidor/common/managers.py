...
class SuperManager:
    ...
    def get_page(self, page_nr=0, max_entries=0, like_search=None, order_by=None, ascendant=True, query=None):
        ...
        Parameters
        ----------
        page_nr : int | Page number to display.
        max_entries : int | max records per page.
        like_search : string, None | Criteria de búsqueda.
        order_by : string, None | Nombre de la columna a ordenar.
        ascendant : bool | Criteria de sentido de orden.
        query:
        Returns
        -------
        dict | Retorna un diccionario en donde la clave objects contiene el reultado del listado.
        """
        if not query:
    ...
    def colums_like(self, entity, like_search):
        ...
        Parameters
        ----------
        entity : class | Clase entidad mapeada o modelo de sqlalchemy.
        like_search : string | Criteria de búsqueda.
        Returns
        -------
    ...
    def order_by(self, query, entity, column_name, ascendant):
        ...
        ascendant : bool | Sentido de orden establecido.
        Returns
        -------
        query
            Consulta sqlalchemy.
    ...
    def many_to_many(self, object):
        if hasattr(object, 'secondary_classes'):
            for item in object.secondary_classes:
                ...
                obj_attr.clear()
                for key in keys:
                    obj_attr.append(self.db.query(item[1]).get(key))
        return object
    ...
    def imagen(self, fileinfo):
        try:
            clave = 't2HQtsNT2bAAAAAAAAAAW56QR0QVIgOOVjbllBsxNKT0-OeH2odTXwcjdvwV6EXA'
            ruta = None

            if clave not in ['S/K', None, '', ' ']:
                cliente = dropbox.Dropbox(clave)
                fname = fileinfo.filename
                extn = os.path.splitext(fname)[1]
                cname = str(uuid.uuid4()) + extn

                cliente.files_upload(fileinfo.body, '/images/' + cname)
                link = cliente.sharing_create_shared_link_with_settings('/images/' + cname, None)
