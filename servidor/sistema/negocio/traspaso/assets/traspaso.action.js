...
var fkorigen = document.getElementById('fkorigen');
var fkdestino = document.getElementById('fkdestino');
...
$('#fkdestino').selectpicker({
    size: 10,
    liveSearch: true,
    liveSearchPlaceholder: 'Buscar sucursal',
    title: 'Seleccione un sucursal'
})
...
function load_detail() {
    obj = JSON.stringify({
        'idsucursal': parseInt($(fkorigen).val())
    })
    $.ajax({
        method: "POST",
...
$('#insert').on('click', function() {
    ...
    if (!notvalid) {
        objeto = JSON.stringify({
            ...
            'fkorigen': $('#fkorigen').val(),
            'fkdestino': $('#fkdestino').val(),
            'detalle' : obtener_detalle()
        })
        console.log(JSON.parse(objeto))

        ajax_call('traspaso_insert', {
            object: objeto,
            _xsrf: getCookie("_xsrf")
...
function edit_item(e) {
    ...
    ajax_call_get('traspaso_update',{
        ...
        $(fkorigen).val(self.fkorigen)
        $(fkorigen).selectpicker('render')
        $(fkorigen).change()
        $(fkdestino).val(self.fkdestino)
        $(fkdestino).selectpicker('render')
...
function set_enable(e) {
    ...
    if (!b) {
        cb_title = "¿Está seguro de que desea dar de baja el traspaso?"
        cb_text = ""
        cb_type = "warning"
    } else {
        cb_title ="¿Está seguro de que desea dar de alta el traspaso?"
        cb_text = ""
        cb_type = "info"
    }
...
function delete_item(e) {
    Swal.fire({
        icon: "warning",
        title: "¿Está seguro de que desea eliminar permanentemente el traspaso?",
        text: "",
        showCancelButton: true,
        allowOutsideClick: false,
        confirmButtonColor: '#1565c0',
