...
class SalidaManager(SuperManager):
    ...
    def insert(self, objeto):
        ...
        b = Bitacora(fkusuario=objeto.user, ip=objeto.ip, accion="Registró salida.", fecha=fecha, tabla="negocio_salida", identificador=a.id)
        super().insert(b)
        return a
    ...
    def update(self, objeto):
        ...
        b = Bitacora(fkusuario=objeto.user, ip=objeto.ip, accion="Modificó salida.", fecha=fecha, tabla="negocio_salida", identificador=a.id)
        super().insert(b)
        return a

    def state(self, id, estado, user, ip):
        x = self.db.query(self.entity).filter(self.entity.id == id).one()
        mensaje = "Habilitó salida" if estado else "Deshabilitó salida"
        x.estado = estado
    ...
    def delete(self, id, user, ip):
        ...
        fecha = BitacoraManager(self.db).fecha_actual()
        b = Bitacora(fkusuario=user, ip=ip, accion="Eliminó salida", fecha=fecha, tabla="negocio_salida", identificador=id)
        super().insert(b)
        self.db.merge(x)
        self.db.commit()
