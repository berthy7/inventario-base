...
$('#fkinventario').selectpicker({
    size: 10,
    liveSearch: true,
    liveSearchPlaceholder: 'Buscar inventario',
    title: 'Seleccione una inventario'
})
...
function load_table(data_tb) {
    var tabla = $(id_table).DataTable({
        ...
        "order": [ [1, 'asc'], [2, 'asc'] ],
        columnDefs: [ { width: '20%', targets: [1] }, { width: '36%', targets: [2] }, { width: '10%', targets: [3, 4] }, { width: '12%', targets: [5, 6] } ],
...
function edit_item(e) {
    ...
    ajax_call_get('inventario_update',{
        ...
        $(fkinventario).val(self.fkinventario)
        $(fkinventario).selectpicker('render')
