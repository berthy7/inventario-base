...
function load_table(data_tb) {
    var tabla = $(id_table).DataTable({
        ...
        "order": [ [0, 'desc'] ],
        columnDefs: [ { width: '35%', targets: [1, 2] }, { width: '15%', targets: [3, 4] } ],
...
function set_enable(e) {
    ...
    if (!b) {
        cb_title = "¿Está seguro de que desea dar de baja el cliente?"
        cb_text = ""
        cb_type = "warning"
    } else {
        cb_title ="¿Está seguro de que desea dar de alta el cliente?"
        cb_text = ""
        cb_type = "info"
    }
...
function delete_item(e) {
    Swal.fire({
        icon: "warning",
        title: "¿Está seguro de que desea eliminar permanentemente el cliente?",
        text: "",
        showCancelButton: true,
        allowOutsideClick: false,
        confirmButtonColor: '#1565c0',
